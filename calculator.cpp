...
// WordNode* FindBeforeNode(WordNode* p, WordNode* head){
// 	WordNode* q = head;
// 	while(q){
// 		if(p == q->next){
// 			return q;
...
WordNode* StrToWordNode(char* s) {
	int len = strlen(s);
	WordNode* head = CreateWordHead();
	for (int i = 0; i < len; i++) {
		char temp = s[i];
		if (temp != ' ')
			InsertWord(head, temp, 0, i + 1);
	}
	return head;
}
...
Type CharType(char c) {
	Type type;
	if (strchr(NUM, c)) { type = Num; }
	else if (strchr(ID, c)) { type = Id; }
	else if (strchr(SIGN, c)) { type = Sign; }
	else if (c == '(') { type = LeftBarcket; }
	else if (c == ')') { type = RightBarcket; }
